...
# AHernandez, 2021-Mar-12, Added track setters and getters with exception handeling
# AHernandez, 2021-Mar-12, Changed the string formatting
# AHernandez, 2021-Mar-12, added counter when tracks are added in a random order
#------------------------------------------#
...
class Track:
    ...
    def position(self, value):
        if type(value) == int:
            if value < 1:
                raise Exception('The position of the track cannot be less than 1')
            self.__position = value
        else:
            raise Exception('The positon must be a base 10 number')
    ...
    def length(self, value):
        if type(value) == str:
            ...
            raise Exception('The lenght of the track must be a string')
    ...
    def get_tracks(self) -> str:
        ...
        while True:
            if len(self.__tracks) < 1:
                print('\nThere are no tracks for this CD\n')
                break
            result = ''
            for counter, track in enumerate(self.__tracks, start=1):
                if track is None:
                    result += f'{counter} No Information for this track\n'
                else:
                    result += str(track) + '\n'
            return result
