...
# AHernandez, 2021-Mar-12, Added track processing logic.  Added some error handeling 
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        while True:
            for row in table:
                if row.cd_id == cd_idx:
                    return row
            raise Exception('The requested CD does not exists')    
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        try:
            ...
            raise Exception('The postiion must be a base 10 number')
