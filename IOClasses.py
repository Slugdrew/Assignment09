...
# AHernandez, 2021-Mar-12, Added track and CD file save and load logic
        ...
        try:
            ...
            with open(file_name_track, 'w') as file:
                for track in lst_Inventory:
                    tracks = track.cd_tracks
                    disc_id = track.cd_id
                    for track in tracks:
                        if track is not None:
                            record = '{},{}'.format(disc_id, track.get_record())
                            file.write(record)
            print(f'The following records were saved to {file_name_CD} and {file_name_track}\n')
